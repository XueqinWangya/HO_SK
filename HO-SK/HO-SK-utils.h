...
void Initialize_vectors(int N, int Iter, double **natural_w_out, double **w_eff_out, double **theta_mem_out) {
    ...
    double *theta_mem = calloc(N * Iter, sizeof(double));  // Cast the void pointer to double pointer N × Iter
...
void Initial_Conditions(int N, double *natural_w, double *theta_mem, double freq_factor, double phase_factor) {
    // Natural frequencies within the interval 
...
void Force_theta(int i, double *aux, double *ki, double *natural_w, int *T_1, int *T_2, int *k_Pairs, int *k_Trios, double sigma_P, double sigma_T, int N, int k_Pairs_med, int k_Trios_med, int *V, double beta) {
    double force_P, force_T;
    int j, k;
    
    force_P = force_T = 0;
    
    // Force from pairs  成对相互作用 
    for(j = 0; j < k_Pairs[i]; j++) {
        force_P += sin(M_PI * (aux[V[N * i + j]] - aux[i]) - beta);
    }
    force_P *= sigma_P / k_Pairs_med;
    
    // Force from trios
    for(k = 0; k < k_Trios[i]; k++) {
        force_T += sin(M_PI * (aux[T_1[i * N + k]] + aux[T_2[i * N + k]] - 2 * aux[i]) - beta);
    }
...
void Evolution(int N, int t, double h, double *theta_mem, double *w_eff, double *natural_w, int *T_1, int *T_2, int *k_Pairs, int *k_Trios, int *V, double sigma_P, double sigma_T, int k_Pairs_med, int k_Trios_med, double beta) {
    int i, j, k;
    ...
    // RK First step
    for(i = 0; i < N; i++) {
        Force_theta(i, aux, k1, natural_w, T_1, T_2, k_Pairs, k_Trios, sigma_P, sigma_T, N, k_Pairs_med, k_Trios_med, V, beta);
        c_aux[i] = theta_mem[t * N + i] + h * k1[i] / 2;
    }

    // RK Second step
    for(i = 0; i < N; i++) {
        Force_theta(i, c_aux, k2, natural_w, T_1, T_2, k_Pairs, k_Trios, sigma_P, sigma_T, N, k_Pairs_med, k_Trios_med, V, beta);
        aux[i] = theta_mem[t * N + i] + h * k2[i] / 2;
    }

    // RK Third step
    for(i = 0; i < N; i++) {
        Force_theta(i, aux, k3, natural_w, T_1, T_2, k_Pairs, k_Trios, sigma_P, sigma_T, N, k_Pairs_med, k_Trios_med, V, beta);
        c_aux[i] = theta_mem[t * N + i] + h * k3[i];
    }

    // RK Fourth step
    for(i = 0; i < N; i++) {
        Force_theta(i, c_aux, k4, natural_w, T_1, T_2, k_Pairs, k_Trios, sigma_P, sigma_T, N, k_Pairs_med, k_Trios_med, V, beta);
...
double r_link(int N, int Iter, int Iter_burn, double *theta_mem, int *k_Pairs, int *k_Trios, int *V, int *T_1, int *T_2) {
    if (theta_mem == NULL || k_Pairs == NULL || k_Trios == NULL || V == NULL || T_1 == NULL || T_2 == NULL) {
        printf("? Null pointer detected in r_link inputs.\n");
        return -1;
    }
    ...
    int t_start = Iter_burn + (Iter - Iter_burn) / 2;
    int t_range = Iter - t_start;
    if (t_range <= 0) {
        printf("? Invalid time range: Iter=%d, Iter_burn=%d\n", Iter, Iter_burn);
        return -1;
    }
    ...
    for (i = 0; i < N; i++) { 
        for (j = 0; j < k_Pairs[i]; j++) {  
            ...
            Real = Im = 0;
            for (tt = t_start; tt < Iter; tt++) {  
                dif = M_PI * (theta_mem[tt * N + i] - theta_mem[tt * N + idx]);
                Real += cos(dif);
                Im += sin(dif);
            }
            rij += sqrt(Real * Real + Im * Im) / t_range;  
            norm_p += 1;    
        }
        ...
        for (k = 0; k < k_Trios[i]; k++) {  
            int idx1 = T_1[i * N + k];
            int idx2 = T_2[i * N + k];
            if (idx1 < 0 || idx1 >= N || idx2 < 0 || idx2 >= N) continue;

            Real = Im = 0;
            for (tt = t_start; tt < Iter; tt++) {
                dif = M_PI * (theta_mem[tt * N + idx1] + theta_mem[tt * N + idx2] - 2 * theta_mem[tt * N + i]);
                Real += cos(dif);
                Im += sin(dif);
            }
            rij += sqrt(Real * Real + Im * Im) / t_range;
    ...
    if ((norm_p + norm_t) == 0) {
        printf("? No links found for r_link calculation.\n");
        return -1;
    }
